...
Publishing is gated on CI: pushing a git tag runs `test`/`phpstan` first (`.github/workflows/ci.yml`), then a `deploy` job (`needs: [test, phpstan]`, only on tag refs) pings Packagist's API to re-fetch the tag — https://packagist.org/packages/ekrouzek/pagination-filters-bundle only updates once both checks pass. That job needs `PACKAGIST_USERNAME`/`PACKAGIST_TOKEN` configured as repo secrets. The repo's old Packagist GitHub webhook (Settings → Webhooks) was removed since it published unconditionally regardless of CI status — don't re-add it, or a broken tag could publish anyway. There's no separate build step.
